...
void Backtrack::TRACE_MAX(Node *max_node, Knapsack &knapsack)
{
    ...
    if ((*max_node->parent).parent == nullptr)
    {
        return;
    }
...
void Backtrack::BACK_TRACE(Node *root, Knapsack &knapsack)
{

    for (int i = 0; i < knapsack.getSize(); i++)
    {
        if ((root->getWeight() - knapsack.getItem(i)->weight) >= 0 && !knapsack.getChosen(i))
        {
            int *d = new int[knapsack.getSize()];

            for (int j = 0; j < knapsack.getSize(); j++)
            {
                d[j] = knapsack.getChosen(j);
            }
            ...
            BACK_TRACE(root->children[i], knapsack);
