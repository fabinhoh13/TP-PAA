...
BranchAndBound::BranchAndBound(int nitems, int weight) {
    this->weight = weight;
...
void BranchAndBound::initialSolution(const Knapsack* instance) {
...
bool BranchAndBound::isComplete(vector<int> items) {
    for (int i : items) {
        if (i == -1) return false;
    }
...
bool BranchAndBound::isConsistent(const Knapsack* instance, vector<int> items) {
    int totalWeight = 0;
    int counter = 0;
...
bool BranchAndBound::isPromissing(const Knapsack* instance, vector<int> items, int idx, int value, int weight) {
    double nValue = value;
...
void BranchAndBound::solve(const Knapsack* instance) {
...
void BranchAndBound::_solve(const Knapsack* instance, vector<int> items, int item, int value, int weight) {
